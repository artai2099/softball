...
export function PublicScoreboard({ initialGame,initialEvents }: { initialGame: Game;initialEvents:GameEvent[] }) {
  ...
  return <main className="livePage" style={{minHeight:"100vh"}}><div style={{maxWidth:760,margin:"auto"}}><div className="brand" style={{marginBottom:28}}><span>G</span>GameDay Softball</div><div className="liveLabel">{game.status==="live"?"● Live scorecast":game.status} · {game.venue||"GameDay Field"}</div><section className="scoreboard"><div><div className="teamLabel">{game.home_name}</div><div className="score">{game.home_score}</div></div><div className="inningBox"><small>{game.half==="top"?"▲ TOP":"▼ BOT"}</small><div style={{fontSize:28,fontWeight:950}}>{game.inning}</div><small>{game.outs} OUT</small></div><div><div className="teamLabel">{game.away_name}</div><div className="score">{game.away_score}</div></div></section><section className="panel"><div className="count"><span>B {game.balls}</span><span>S {game.strikes}</span><span>O {game.outs}</span><span>P {(game.balls + game.strikes)}</span></div></section><PlayFeed events={events}/>{game.status==="live"&&<LiveRoom gameId={game.id} role="viewer" />}<p style={{color:"#9fb1c8",textAlign:"center",fontSize:12}}>Scores update automatically. No account is required for public games.</p></div></main>;
}
