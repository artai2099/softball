...
export function ScoringConsole({ initialGame,initialEvents,canScore }: { initialGame: Game;initialEvents:GameEvent[];canScore:boolean }) {
  ...
  return <div className="livePage"><div className="liveGrid"><div>
    ...
    <section className="panel"><div className="pageHead"><div><div className="liveLabel">Pitch count</div><div className="count"><span>B {game.balls}</span><span>S {game.strikes}</span><span>O {game.outs}</span><span>P {(game.balls + game.strikes)}</span></div></div><div>{canScore&&<><button className="button secondary" disabled={saving} onClick={()=>void undo()}>Undo</button> <button className="button red" disabled={saving||game.status==="final"} onClick={()=>void finish()}>Finish</button></>} {game.visibility==="public"?<a className="button secondary" href={`/watch/${game.public_id}`}>Viewer page</a>:<span className="notice">Private game</span>}</div></div></section>
