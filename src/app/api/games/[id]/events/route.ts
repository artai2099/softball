...
import { scoreEventSchema } from "@/lib/scoring";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = scoreEventSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Invalid scoring event", issues: parsed.error.issues }, { status: 400 });
